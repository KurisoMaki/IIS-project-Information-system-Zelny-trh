        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                @foreach($farmers as $farmer)
                    <tr>
                        <td>
                            <a href="{{ route('uzivatele.show', $farmer->prihlasovaci_jmeno) }}">{{ $farmer->jmeno }}</a>
                        </td>
                        <td>{{ $farmer->email }}</td>
                        {{-- <td>
                            <a href="{{ route('uzivatele.edit', $farmer->prihlasovaci_jmeno) }}" class="btn btn-warning btn-sm">Upravit</a>
                            <form action="{{ route('uzivatele.destroy', $farmer->prihlasovaci_jmeno) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Opravdu chcete tohoto farmáře smazat?')">Smazat</button>
                            </form>
                        </td> --}}
                        <td>
                            <a href="{{ route('uzivatele.show', $farmer) }}" class="btn btn-info btn-sm">Zobrazit</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
            ...
            <tbody>
                @foreach($users as $user)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('uzivatele.show', $user) }}" class="btn btn-info btn-sm">Zobrazit</a>

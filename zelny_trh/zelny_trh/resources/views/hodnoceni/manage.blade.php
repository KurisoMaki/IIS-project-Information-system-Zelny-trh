            ...
            @foreach($hodnoceni as $item)
                <tr>
                    ...
                    <td>
                        ...
                        <a href="{{ route('hodnoceni.edit', $item->id_hodnoceni) }}" class="btn btn-warning btn-sm">Upravit</a>
                        <form action="{{ route('hodnoceni.destroy', $item->id_hodnoceni) }}" method="POST" style="display:inline-block;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Opravdu chcete toto hodnocení smazat?')">Smazat</button>
                        </form>
                    </td>
                </tr>
            @endforeach
